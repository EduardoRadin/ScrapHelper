...
public class Main {
    ...
    private static void adicionarCEP(Scanner scanner) {
        try {
            System.out.println("Municípios disponíveis:");
            List<Municipio> municipios = MunicipioDAO.getInstancia().listarTodos();
            if (municipios.isEmpty()) {
                System.out.println("Nenhum município cadastrado. Cadastre um município primeiro!");
                return;
            }
            for (Municipio mun : municipios) {
                System.out.println("Código: " + mun.getMun_cod() + " - Nome: " + mun.getMun_nome());
            }
            ...
            Municipio municipio = MunicipioDAO.getInstancia().buscarPorId(cod_mun);
            if (municipio == null) {
                System.out.println("Erro: Município com código " + cod_mun + " não encontrado!");
                return;
            }
            
            System.out.print("CEP (apenas números, ex: 89874000): ");
            String cep_cod = scanner.nextLine();
            
            // Validar formato do CEP (apenas números, 8 dígitos)
            if (!cep_cod.matches("\\d{8}")) {
                System.out.println("Erro: CEP deve conter exatamente 8 dígitos numéricos!");
                return;
            }
    ...
    private static void adicionarEnderecoFun(Scanner scanner) {
        try {
            System.out.println("IMPORTANTE: Certifique-se de que os códigos informados existem no banco de dados!");
            System.out.println("Ordem recomendada: 1) Estado, 2) Município, 3) CEP, 4) Tipo de Endereço, 5) Cargo, 6) Funcionário");
            ...
            System.out.print("CEP (apenas números, ex: 89874000): ");
            String codCep = scanner.nextLine();
            
            // Validar formato do CEP (apenas números, 8 dígitos)
            if (!codCep.matches("\\d{8}")) {
                System.out.println("Erro: CEP deve conter exatamente 8 dígitos numéricos!");
                return;
            }
    ...
    private static void adicionarEstado(Scanner scanner) {
        try {
            ...
            Estado estado = new Estado(0, sigla, nome); 
    ...
    private static void adicionarFuncionario(Scanner scanner) {
        try {
            ...
            List<Cargo> cargos = CargoDAO.getInstancia().listarTodos();
            if (cargos.isEmpty()) {
                System.out.println("Nenhum cargo cadastrado. Cadastre um cargo primeiro!");
                return;
            }
            for (Cargo cargo : cargos) {
                System.out.println("Código: " + cargo.getCargo_cod() + " - Descrição: " + cargo.getcargo_descricao());
            }
            ...
            Cargo cargo = CargoDAO.getInstancia().buscarPorId(codCargo);
            if (cargo == null) {
                System.out.println("Erro: Cargo com código " + codCargo + " não encontrado!");
                return;
            }
    ...
    private static void adicionarMunicipio(Scanner scanner) {
        try {
            ...
            List<Estado> estados = EstadoDAO.getInstancia().listarTodos();
            if (estados.isEmpty()) {
                System.out.println("Nenhum estado cadastrado. Cadastre um estado primeiro!");
                return;
            }
            for (Estado est : estados) {
                System.out.println("Código: " + est.getEstado_cod() + " - Nome: " + est.getEstado_nome() + " (" + est.getEstado_sigla() + ")");
            }
            ...
            Estado estado = EstadoDAO.getInstancia().buscarPorId(codEstado);
            if (estado == null) {
                System.out.println("Erro: Estado com código " + codEstado + " não encontrado!");
                return;
            }
    ...
    private static void adicionarPessoa(Scanner scanner) {
        try {
            ...
            if (tipo.equalsIgnoreCase("F")) {
                System.out.print("Data de Nascimento (AAAA-MM-DD - obrigatória para Pessoa Física): ");
                dataNas = scanner.nextLine();
                if (dataNas.isEmpty()) {
                    throw new IllegalArgumentException("Data de nascimento é obrigatória para Pessoa Física.");
                }
                try {
                    LocalDate.parse(dataNas); // Validar o formato
                } catch (DateTimeParseException e) {
                    throw new IllegalArgumentException("Formato de data inválido. Use AAAA-MM-DD.");
                }
            } else if (tipo.equalsIgnoreCase("J")) {
                System.out.println("Data de Nascimento não se aplica a Pessoa Jurídica.");
            }
